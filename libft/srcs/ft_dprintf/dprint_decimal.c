...
/*   Updated: 2025/10/16 00:53:47 by aomatsud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_dprintf.h"

static int	ft_dputnbr_fd_unsigned_error(unsigned int n, int fd)
{
	char	c;

	if (n >= 10)
	{
		if (ft_dputnbr_fd_unsigned_error(n / 10, fd) == -1)
			return (-1);
	}
	c = n % 10 + '0';
	if (write(fd, &c, 1) == -1)
...
{
	if (ft_dputnbr_fd_unsigned_error(nbr, fd) == -1)
		return (-1);
